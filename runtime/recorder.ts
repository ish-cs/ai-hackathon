...
export class Recorder {
  ...
  async start(url: string): Promise<{ liveViewUrl?: string }> {
    // Self-heal: a prior session may linger if its window was closed manually or a stop()
    // failed. Tear it down instead of refusing forever — otherwise the recorder wedges in
    // "already recording" and every Record click 500s.
    if (this.opened) {
      try {

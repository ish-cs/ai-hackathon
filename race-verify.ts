...
ws.on("message", (d) => {
  ...
  switch (e.kind) {
    ...
    case "run_done":
      ...
      if (stDone >= ROUNDS && miDone >= ROUNDS) finish();
      break;
  }
});
...
function finish() {
  ...
  console.log("======================================");
  process.exit(broke && stDone >= ROUNDS && miDone >= ROUNDS ? 0 : 1);
}
